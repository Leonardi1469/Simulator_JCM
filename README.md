...
The Hamiltonian is written in the rotating-wave approximation (RWA), which neglects the counter-rotating terms $a\sigma_-$ and $a^\dagger\sigma_+$. Its use requires the effective coupling $g\sqrt{n+1}$ to be small compared with $\omega_c+\omega_{eg}$ for the photon numbers populated during the evolution. The interface displays indicative values for the coupling and detuning and warns when the selected parameters require closer examination. These numerical indicators do not define a strict boundary for the approximation.
